...
1件の登録は、完全な `[[link]]` ブロック1つで、文字列の `link` と `target` の2項目だけを記述します。両方とも絶対パスが必要です。`version` やその他のトップレベルの設定はありません。空ファイルは登録0件を表します。

`list` は、不正なパスや重複した登録も含めて、ファイルの記載順に表示します。例えば、手編集した `target = "PhD"` も一覧には表示されます。`check`・`scan`・変更操作は、リンクの検査や変更の前に管理ファイル全体を検証するため、この target は絶対パスでないとして拒否します。パスのエラーには管理ファイルの場所・項目番号・不正なフィールドを示し、意図したパスを推測することはありません。
